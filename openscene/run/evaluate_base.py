...
def precompute_text_related_properties(labelset_name):
    ...
    elif 'test' in labelset_name:
        labelset = list(TEST)
        palette = get_palette(colormap='test')
    ...
    with torch.no_grad():
        ...
        for rep_i in range(args.test_repeats):
            ...
            for i, (coords, feat, label, feat_3d, mask, inds_reverse) in enumerate(tqdm(val_data_loader)):
                ...
                if vis_input:
                    input_color = torch.load(val_data_loader.dataset.data_paths[i], weights_only=False)[1]
                ...
                if vis_pred:
                    ...
                    visualize(origional_pcl,pred_label_color,logits_pred)
