...
class ValueMapVisualizer:
    ...
    def visualize(self, info, show=False, save=True):
        ...
        waypoints_world = np.array([p for p in info['path_world']])
        ...
        if info is not None:
            self.update_bounds(info['workspace_bounds_min'],info['workspace_bounds_max'])
            map_to_visualize='costmap'
            # map_to_visualize='obstacle_map'
            # map_to_visualize='target_map'
            # map_to_visualize='None'
            # plot costmap
            if map_to_visualize in planner_info:
                map = planner_info[map_to_visualize][::self.downsample_ratio, ::self.downsample_ratio, ::self.downsample_ratio]
                ...
                fig_data.append(go.Volume(x=x.flatten(), y=y.flatten(), z=z.flatten(), value=map.flatten(), isomin=0, isomax=1, opacity=self.costmap_opacity, surface_count=self.costmap_surface_count, colorscale='Jet', showlegend=True, showscale=False))
            plot_avoidance_map=False
            avoidance_map_dawnsample=self.downsample_ratio*2
            if plot_avoidance_map:
                map = info['avoidance_map'][::avoidance_map_dawnsample, ::avoidance_map_dawnsample, ::avoidance_map_dawnsample]
                skip_ratio = (self.workspace_bounds_max - self.workspace_bounds_min) / (self.map_size / avoidance_map_dawnsample)
                x, y, z = np.mgrid[self.workspace_bounds_min[0]:self.workspace_bounds_max[0]:skip_ratio[0],
                                self.workspace_bounds_min[1]:self.workspace_bounds_max[1]:skip_ratio[1],
                                self.workspace_bounds_min[2]:self.workspace_bounds_max[2]:skip_ratio[2]]
                grid_shape = map.shape
            ...
            if 'start_pos' in info:
                fig_data.append(go.Scatter3d(x=[start_pos_world[0]], y=[start_pos_world[1]], z=[start_pos_world[2]], mode='markers', name='start', marker=dict(size=12, color='blue')))
            # plot target as dots extracted from target_map
            if 'targets_world' in info:
                targets_world = info['targets_world']
                fig_data.append(go.Scatter3d(x=targets_world[:, 0], y=targets_world[:, 1], z=targets_world[:, 2], mode='markers', name='target', marker=dict(size=12, color='green')))
            for aabb in info['obstacle_bounding_box_world']:
        ...
        env_mesh=info['env_mesh']
        scene_pcd = info["scene_pcd"]


        scene_points=info["scene_points"]
        scene_points_colors=info["scene_points_colors"]
        # visualize scene points
        print(f"point cloud count: {scene_points.shape[0]}")
        ...
        #     if scene_points_colors is not None:
        #         scene_points_colors = scene_points_colors[resample_idx]
        # scene_points_colors[:,:]=[100,100,200]
        fig_data.append(go.Scatter3d(x=scene_points[:, 0], y=scene_points[:, 1], z=scene_points[:, 2],
                                    mode='markers', marker=dict(size=3, color='brown', opacity=1)))
