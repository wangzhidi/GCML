...
class PathPlanner:
    ...
    def optimize(self, start_pos: np.ndarray, target_map: np.ndarray, obstacle_map: np.ndarray, object_centric=False):
        ...
        for i in range(self.config.max_steps):
            ...
            # add inertia mechenism to prevent being trapped in coners
            ...
            if previous_pos is not None:
                ...
                inertia = np.dot(all_nearby_voxels - current_pos, direction)
                inertia_weight = 0.0002  # inertia weight
                nearby_score -= inertia_weight * inertia
            # reduce up and down voxels' score to prevent going up and down
            z_penalty_weight=0.1
            z_diff = np.abs(all_nearby_voxels[:, 2] - current_pos[2])
            nearby_score += z_penalty_weight * z_diff
