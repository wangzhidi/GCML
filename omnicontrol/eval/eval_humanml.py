...
def evaluate_control(motion_loaders, file):
    ...
    # ~average distance between generated traj and hint traj
...
def evaluate_fid(eval_wrapper, groundtruth_loader, activation_dict, file):
    ...
    # ~remove embeddings that contains NaN
    gt_motion_embeddings = gt_motion_embeddings[~np.isnan(gt_motion_embeddings).any(axis=1)]
    print(gt_motion_embeddings.shape)
    # ~mu=μ
    ...
    for model_name, motion_embeddings in activation_dict.items():
        ...
        # ~remove embeddings that contains NaN
        motion_embeddings = motion_embeddings[~np.isnan(motion_embeddings).any(axis=1)]
        print(motion_embeddings.shape)
