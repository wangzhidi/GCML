...
class GaussianDiffusion:
    ...
    def local2global(self, matrix, R, t):
        # Perform the transformation in a vectorized manner
        return torch.matmul(matrix, R.T) + t
    def gradients(self, x, hint, mask_hint, joint_ids=None):
        with torch.enable_grad():
            ...
            joint_pos = recover_from_ric(x_, n_joints)      #[3,120,22,3]
            if n_joints == 21:
            ...
            for i in range(joint_pos.shape[0]):
                for j in range(len(self.R)):
                    R=th.from_numpy(self.R[-j-1][i]).to(device=joint_pos.device).float()
                    t=th.from_numpy(self.t[-j-1][i]).to(device=joint_pos.device).float()
                    joint_pos[i] = torch.matmul(joint_pos[i].clone(), R.T) + t
            loss = torch.norm((joint_pos - hint) * mask_hint, dim=-1)   #joint_pos and hint here is close to local xyz coordinates
    ...
    def guide(self, x, t, model_kwargs=None, t_stopgrad=-10, scale=.5, n_guide_steps=10, train=False, min_variance=0.01):
        ...
        self.R=model_kwargs['y']['R']
        self.t=model_kwargs['y']['t']
