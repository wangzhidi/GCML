...
import mesh_to_sdf
from test_sdf_grid import MeshSDFCalculator
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        mesh_path = Path("collisionTest.obj")
        self.sdf_calculator = MeshSDFCalculator(mesh_path)
    ...
    # -caculate loss and gradients with predicted x and input hint
    def gradients(self, x, hint, mask_hint, joint_ids=None):
        with torch.enable_grad():
            x.requires_grad_(True)
            # -contiguous is used to ensure x is contiguous stored in memory
            x_ = x.permute(0, 3, 2, 1).contiguous()
            x_ = x_.squeeze(2)
            x_ = x_ * self.std + self.mean
            n_joints = 22 if x_.shape[-1] == 263 else 21
            joint_pos = recover_from_ric(x_, n_joints)      #[3,196,22,3]
            if n_joints == 21:
            ...
            joint_pos=joint_pos.view(3,196*1,3)
            sdf_value, gradient = self.sdf_calculator.calculate_sdf_value_and_gradient(joint_pos)
            sdf_loss=196.0-sdf_value

            # loss = torch.norm((joint_pos - hint) * mask_hint, dim=-1)
            grad = torch.autograd.grad([sdf_loss.sum()], [x])[0]
            # the motion in HumanML3D always starts at the origin (0,y,0), so we zero out the gradients for the root joint
            grad[..., 0] = 0
            x.detach()
        return sdf_loss, grad
    ...
    #         # loss = torch.norm((joint_pos - hint) * mask_hint, dim=-1)
    #         loss = torch.norm((joint_pos - hint) * mask_hint, dim=-1) #[bs,196,22,1]
    #         pelvis_pos=joint_pos[:,:,0,:]
    #         reshaped_pos=pelvis_pos.reshape([-1, 3])
            
    #         numpy_pos=reshaped_pos.detach().cpu().numpy()
    #         mesh = trimesh.load('collisionTest.obj')
    #         newloss=mesh_to_sdf.mesh_to_sdf(mesh, numpy_pos, surface_point_method='scan', 
    #                     sign_method='normal', bounding_radius=None, 
    #                     scan_count=10, scan_resolution=40, 
    #                     sample_point_count=100000, normal_sample_count=11)

    #         # newloss=1.0-1.0 / (1.0 + torch.exp(-0.1 * (loss.sum() - 500.0)))
    #         # print("loss:{}  ,newloss:{}".format(loss.sum(),newloss.item()))

    #         grad = torch.autograd.grad([newloss], [x])[0]
    #         # the motion in HumanML3D always starts at the origin (0,y,0), so we zero out the gradients for the root joint
    #         grad[..., 0] = 0
    #         x.detach()
    #     return loss, grad
    ...
    def guide(self, x, t, model_kwargs=None, t_stopgrad=-10, scale=.5, n_guide_steps=10, train=False, min_variance=0.01):
        ...
        # -mask_hint is bool tensor that determines if every frame, every joint, every axis needs process 
