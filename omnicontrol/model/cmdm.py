...
# -detach a module's parameters to a new one and initiate them as zero
...
class CMDM(torch.nn.Module):
    ...
    # -mask some cond in a batch, make them zero
    def mask_cond(self, cond, force_mask=False):
        bs, d = cond.shape
        if force_mask:
            return torch.zeros_like(cond)
        elif self.training and self.cond_mask_prob > 0.:
            # -bernoulli returns 1 for input in (0.5,1) and 0 for input less than 0.5
            mask = torch.bernoulli(torch.ones(bs, device=cond.device) * self.cond_mask_prob).view(bs, 1)  # 1-> use null_cond, 0-> use real cond
            return cond * (1. - mask)
        else:
            return cond
        
    # -get latent vector from input text using CLIP, max length is 77 by default
    ...
    # -cmdm refers to the copied part
    ...
    # -forward that combines the above two, if spatial hint exists, cmdm x and y is passed
    # -to cmdm, else create zero tensor and pass it to cmdm.
    ...
    # -_apply is used for applying something to a module's all sub modules
    def _apply(self, fn):
        super()._apply(fn)
        self.rot2xyz.smpl_model._apply(fn)

    # -train is used to set a module at training mode
    def train(self, *args, **kwargs):
        super().train(*args, **kwargs)
        self.rot2xyz.smpl_model.train(*args, **kwargs)

# -convert input hints into latent
...
# -refers to MLP in MDM paper figure
...
# -refer to the x_t to Linear part in MDM figure
...
# refer to the Linear to x_0 part in MDM figure, transfers output to origional data dimention
...
# -action embedding in MDM, perhaps not used
