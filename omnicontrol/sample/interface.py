...
for i in range(0,4):
    ...
    if i==0:
        texts=["a person walks forward to the right and then to the left",]
        texts=texts+texts+texts
        hints=np.zeros((1,120,22,3)) 
        hints[0,80:100,0,:]=[5.2,6.8,1.0]
        hints[0,115:120,0,:]=[4.2,7.8,1.0]
        hints=hints[:,:,:,[0,2,1]]        #now is x,z,y in blender when using 
        hints[..., 0] = -hints[..., 0]
        hints = np.concatenate([hints,hints,hints], axis=0)
        n_frames=120
    if i==1:
        texts=["a person turns back",]
        texts=texts+texts+texts
        hints=np.zeros((1,60,22,3))
        hints[0,30:60,0,:]=[3.8,7.5,1.0]
        # hints[0,70:80,21,:]=[3.5,7.2,1.0]
        hints=hints[:,:,:,[0,2,1]]        #now is x,z,y in blender when using 
        hints[..., 0] = -hints[..., 0]
        hints = np.concatenate([hints,hints,hints], axis=0)
        n_frames=60
    if i==2:
        texts=["a person picks up something on the table",]
        texts=texts+texts+texts
        hints=None
        n_frames=60
    if i==3:
        texts=["a person puts his hands near his mouth",]
        texts=texts+texts+texts
        hints=None
        n_frames=90
