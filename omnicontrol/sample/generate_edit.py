...
def main():
    ...
    # iterator = iter(data)
    # input_motions, _ = next(iterator)
    # input_motions = input_motions.to(dist_util.dev())   #(3,263,1,196)
    input_motions=np.load("raw_sample.npy")
    input_motions=torch.from_numpy(input_motions)
    input_motions=input_motions.to(dist_util.dev())

    texts = ["a person walks forward"] * args.num_samples
    ...
    for rep_i in range(args.num_repetitions):
        ...
        np.save("raw_sample.npy", raw_sample)

        sample = sample[:, :263]            #(3,263,1,196)
        # Recover XYZ *positions* from HumanML3D vector representation
        if model.data_rep == 'hml_vec':
            n_joints = 22 if sample.shape[1] == 263 else 21
            sample = data.dataset.t2m_dataset.inv_transform(sample.cpu().permute(0, 2, 3, 1)).float()   #(3,1,196,263)
            sample = recover_from_ric(sample, n_joints)     #(3,1,196,22,3)
            sample = sample.view(-1, *sample.shape[2:]).permute(0, 2, 3, 1)     #(3,22,3,196)
