...
def generate_motion(n_frames, out_path, texts, hints,index):
    global R,t
    if out_path == '':
    ...
    # use MDM's motion inbetweening method
    ...
    if os.path.exists(last_xyz_sample_path):
        ...
        loaded_raw_sample=torch.from_numpy(loaded_raw_sample)
    ...
    #             hints[i,:,:,:] = global2local(hints[i,:,:,:], R[-j-1][i], t[-j-1][i])
    ...
    #     #normalize
    #     mask = hint != 0
    #     normalized_hint = np.zeros_like(hint)
    #     broadcasted_mean = np.broadcast_to(raw_mean.reshape(1, 22, 3), hint.shape)
    #     broadcasted_std = np.broadcast_to(raw_std.reshape(1, 22, 3), hint.shape)
    #     normalized_hint[mask] = (hint[mask] - broadcasted_mean[mask]) / broadcasted_std[mask]
